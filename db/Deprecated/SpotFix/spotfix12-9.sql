-- This fix adds all group threads to display in get_message_threads()

CREATE OR REPLACE FUNCTION public.db_user_has_group_read_permission(
    "inGroupID" integer,
    "inUserID" integer)
  RETURNS boolean AS
$BODY$DECLARE
  GroupPermission integer;
BEGIN
  GroupPermission = db_get_group_permissions("inGroupID","inUserID");
  IF GroupPermission > 0 AND  GroupPermission != 2 THEN
    RETURN TRUE;
  ELSE
    RETURN FALSE;
  END IF;
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION public.db_user_has_group_read_permission(integer, integer)
  OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.db_user_has_group_read_permission(integer, integer) TO public;
GRANT EXECUTE ON FUNCTION public.db_user_has_group_read_permission(integer, integer) TO postgres;
GRANT EXECUTE ON FUNCTION public.db_user_has_group_read_permission(integer, integer) TO oahu_user;
